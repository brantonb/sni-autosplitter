...
func NewCLI(logger *logrus.Logger, gamesDir, runsDir string, enableManualOps bool, sniHost string, sniPort int, liveSplitPort int, tlsCert, tlsKey string) *CLI {
	return &CLI{
		logger:          logger,
...
func (c *CLI) Start(runName string) error {
	...
	engineController := NewEngineController(c.logger, c, c.enableManualOps, c.liveSplitPort, c.tlsCert, c.tlsKey)
